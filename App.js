  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttons}>
        ...
        <TouchableOpacity onPress={() => handleInput(button)} key={button} style={[styles.button, {backgroundColor: '#2F0B3A'}]}>
          <Text style={[styles.textButton, {color: "white", fontSize: 30}]}>{button}</Text>
        </TouchableOpacity>
          : // Mapeamento dos outros botões   #4B088A
          <TouchableOpacity onPress={() => handleInput(button)} key={button} style={styles.button}>
            <Text style={[styles.textButton, {color: typeof(button) === 'number' ? 'white': '#A4A4A4'}]}>{button}</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
